var index = new Vue({
  ...
  methods:{
    getinit(){
      ...
      var lasna = localStorage.getItem("dept_id");
      var _this=this;
      pub._InitAxios({
        _url:pub._url,
              ...
              cbk:function cbk(res){
                if(res.stateCode=="200"){
                  ...
                  for(var i = 0 ;i <suvl.length; i++){
                    _this.form.contractlist.push({name:suvl[i].uid,url:suvl[i].url});
    ...
    handleAvatarSuccess(res, file) {
      ...
      var goos_count={
        ...
        goods_img:pub._url+res.data.files[0],
      }
    ...
    handleLogoSuccess(res, file){
      console.log(res);
      this.didialogImageUrl=pub._url+res.data.files[0];
    },
    //轮播图  图片成功回调
    handlelunSuccess(res, file){
      console.log(res, file);
      var imgu=file.response.data.files[0];
      this.Lunboimage.push(pub._url+imgu);
      this.form.contractlist.push({name:file.uid,url:pub._url+imgu})
    },
    ...
    handleRemove(file, fileList) {
      ...
      this.Lunboimage=[];
      this.form.contractlist=[];
      if(fileList.length!=0){
        for(let i=0; i<fileList.length;i++){
          var iu=fileList[i].url;
          this.Lunboimage.push(iu);
          this.form.contractlist.push({name:fileList[i].uid,url:iu})
        }
      }else{
        for(let i=0; i<fileList.length;i++){
          var iu=fileList[i].response.data.files[0];
          this.Lunboimage.push(pub._url+iu);
          this.form.contractlist.push({name:fileList[i].uid,url:fileList[i].url})
        }
      }
    ...
    save(){
      ...
      if(this.Logo==undefined || typeof(this.Logo)=='number'){
        this.Logo={};
      };

      if(this.goods_list==undefined || typeof(this.goods_list)=='number'){
        this.goods_list=[];
      }

      if(this.Lunboimage==undefined || typeof(this.Lunboimage)=='number' ){
        this.Lunboimage=[];
      }
      ...
      //导航栏
      this.Navigation={
        // 'value_shou':this.value_shou,
        'value_shang':this.value_shang,
      }

      // console.log(this.value_shou,this.value_shang);
      //店铺logo
      this.Logo={
      ...
      this.experiment_module_data={
        ...
        'Logo':this.Logo,
        'Navigation':this.Navigation,//导航
        'Lunboimage':this.Lunboimage,//轮播图
        'Contractlist':this.form.contractlist,//轮播图回显
    ...
    allsave(){
      ...
      if(lasna==null || lasna=='' || lasna==undefined){
        this.editor(0);
